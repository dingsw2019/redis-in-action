...
class AdClassRemark {
    ...
    private function record_targeting_result($target_id,$ad_id,$words){
        ...
        echo "-------曝光记录开始-----------" . PHP_EOL;
        echo "logId:[{$target_id}],adId:[{$ad_id}]" . PHP_EOL;
        $pipe = $this->conn->pipeline();
        //记录某类广告曝光数
        echo "hget " .AdRedisKey::ad_type_key() . $ad_id . PHP_EOL;
        $ad_type = $this->conn->hget(AdRedisKey::ad_type_key(),$ad_id);
        echo "incr " . AdRedisKey::ad_exposure_key($ad_type) . PHP_EOL;
        $pipe->incr(AdRedisKey::ad_exposure_key($ad_type));

        //记录匹配的关键词,记录关键词的曝光数和某广告曝光数
        echo "smembers " . AdRedisKey::ad_word_key($ad_id) . PHP_EOL;
        $ad_words = $this->conn->smembers(AdRedisKey::ad_word_key($ad_id));
        $matched_words = array_intersect($words,$ad_words);
        if($matched_words){
            echo "sadd " . AdRedisKey::log_word_key($target_id) . " " . implode(" ",$matched_words) . PHP_EOL;
            $pipe->sadd(AdRedisKey::log_word_key($target_id),$matched_words);
            echo "expire " . AdRedisKey::log_word_key($target_id) . " " . self::TTL_900 . PHP_EOL;
            $pipe->expire(AdRedisKey::log_word_key($target_id),self::TTL_900);
            foreach($matched_words as $word){
                echo "zincrby " . AdRedisKey::word_exposure_key($ad_id) . " " . self::INCR_VALUE . " \"" .$word ."\"". PHP_EOL;
                $pipe->zincrby(AdRedisKey::word_exposure_key($ad_id),self::INCR_VALUE,$word);
            }
            echo "zincrby " . AdRedisKey::word_exposure_key($ad_id) . " " . self::INCR_VALUE . " \"" . self::AD_EXPOSURE_MEMBER ." \"". PHP_EOL;
    ...
    public function record_click($target_id,$ad_id){
        ...
        //记录某类广告点击/动作数
        echo "incr {$type_key}" . PHP_EOL;
        $pipe->incr($type_key);

        //记录关键词点击/动作数和某广告点击/动作数
        echo "smembers " . AdRedisKey::log_word_key($target_id) . PHP_EOL;
        $words = $this->conn->smembers(AdRedisKey::log_word_key($target_id));
        foreach($words as $word){
            echo "zincrby " . $word_key . " " . self::INCR_VALUE . " \"" .$word ."\"". PHP_EOL;
            $pipe->zincrby($word_key,self::INCR_VALUE,$word);
        }
        echo "zincrby " . $word_key . " " . self::INCR_VALUE . " \"" . self::AD_CLICK_MEMBER ."\"". PHP_EOL;
    ...
    private function update_cpms($ad_id){
        ...
        list($ad_type,$ad_cpm,$words) = $this->conn->pipeline()
                ...
                ->execute();
        echo "hget " . AdRedisKey::ad_type_key() . " " . $ad_id . PHP_EOL;
        echo "zscore " . AdRedisKey::ad_cpm_key() . " " . $ad_id . PHP_EOL;
        echo "smembers " . AdRedisKey::ad_word_key($ad_id) . PHP_EOL;
        echo "广告类型:[ {$ad_type} ],cpm:[ {$ad_cpm} ],关键词:[ ". implode(",",$words) ." ]".PHP_EOL;
        ...
        list($type_exposure,$type_click) = $this->conn->pipeline()
            ...
            ->get($click_key)
            ->execute();
        echo "get " . AdRedisKey::ad_exposure_key($ad_type) . PHP_EOL;
        echo "get " . $click_key . PHP_EOL;
        $this->AVERAGE_PER_1K[$ad_type] = intval(self::ECPM_BASE_NUMBER * ($type_click ?: 1) / ($type_exposure ?: 1));
        echo "某类广告曝光数:[ {$type_exposure} ],点击数:[ {$type_click} ],平均千次点击数:[ {$this->AVERAGE_PER_1K[$ad_type]} ]" . PHP_EOL;
        ...
        list($ad_exposure,$ad_click) = $this->conn->pipeline()
            ...
            ->execute();
        echo "zscore " . AdRedisKey::word_exposure_key($ad_id) . " \"" . self::AD_EXPOSURE_MEMBER ."\"". PHP_EOL;
        echo "zscore " . $word_click_key . " \"" . self::AD_CLICK_MEMBER ."\"". PHP_EOL;

        $pipe = $this->conn->pipeline();
        if(!$ad_click){
            $ad_ecpm = $this->conn->zscore(AdRedisKey::ad_ecpm_key(),$ad_id);
            echo "zscore " . AdRedisKey::ad_ecpm_key() . $ad_id. PHP_EOL;
        }else{
            ...
            echo "zadd " . AdRedisKey::ad_ecpm_key() . " " . $ad_ecpm . " " . $ad_id . PHP_EOL;
        }
        echo "某广告的曝光数:[{$ad_exposure}],点击数:[{$ad_click}],ecpm:[{$ad_ecpm}]" . PHP_EOL;
        ...
        foreach($words as $word){
            list($word_exposure,$word_click) = $this->conn->pipeline(function ($pipe) use ($ad_id,$word,$word_click_key){
                ...
                echo "zscore " . AdRedisKey::word_exposure_key($ad_id) . " \"" . $word ."\"". PHP_EOL;
                $pipe->zscore($word_click_key,$word);
                echo "zscore " . $word_click_key . " \"" . $word ."\"". PHP_EOL;
            });
            $word_exposure = $word_exposure ?: 1;
            $word_click = $word_click ?: 0;
            ...
            echo "关键词[{$word}]曝光数:[{$word_exposure}],点击数:[{$word_click}],ecpm:[{$word_ecpm}],weight:[{$word_weight[$word]}]" . PHP_EOL;
        }
        if($word_weight){
            $this->conn->pipeline(function($pipe) use ($ad_id,$word_weight){
                foreach($word_weight as $word => $weight){
                    ...
                    echo "zadd " . AdRedisKey::word_weight_key($word) . " " . $weight . " " . $ad_id . PHP_EOL;
                }
            });
        }

        echo "-------更新ECPM结束-----------" . PHP_EOL;
    }
}